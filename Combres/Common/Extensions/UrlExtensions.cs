...
namespace Combres
{
	...
	internal static class UrlExtensions
	{
		...
		public static Uri GetPublicFacingBaseUrl(this HttpContext context)
		{
			...
			System.Collections.Specialized.NameValueCollection headers = request.ServerVariables;
			if (headers["HTTP_HOST"] != null)
			{
				string scheme = IsSsl(headers) ? "https" : "http";
				return new Uri(scheme + Uri.SchemeDelimiter + headers["HTTP_HOST"]);
			}
			return request.Url;
		}

		private static bool IsSsl(System.Collections.Specialized.NameValueCollection headers)
		{
			bool isSsl =
				(headers["HTTP_IS_SSL"] != null && headers["HTTP_IS_SSL"].ToLower().Equals("true")) ||
				(headers["HTTP_X_FORWARDED_PROTO"] != null && headers["HTTP_X_FORWARDED_PROTO"].ToLower().Equals("https"));
